...
static constexpr uint16_t DELAY_BATTERY_STATE_TOPIC_MS =
    AP_DDS_DELAY_BATTERY_STATE_TOPIC_MS;
...
static constexpr uint16_t DELAY_LOCAL_POSE_TOPIC_MS =
    AP_DDS_DELAY_LOCAL_POSE_TOPIC_MS;
#endif // AP_DDS_LOCAL_POSE_PUB_ENABLED
#if AP_DDS_LOCAL_VEL_PUB_ENABLED
static constexpr uint16_t DELAY_LOCAL_VELOCITY_TOPIC_MS =
    AP_DDS_DELAY_LOCAL_VELOCITY_TOPIC_MS;
#endif // AP_DDS_LOCAL_VEL_PUB_ENABLED
#if AP_DDS_AIRSPEED_PUB_ENABLED
static constexpr uint16_t DELAY_AIRSPEED_TOPIC_MS =
    AP_DDS_DELAY_AIRSPEED_TOPIC_MS;
#endif // AP_DDS_AIRSPEED_PUB_ENABLED
#if AP_DDS_GEOPOSE_PUB_ENABLED
static constexpr uint16_t DELAY_GEO_POSE_TOPIC_MS =
    AP_DDS_DELAY_GEO_POSE_TOPIC_MS;
...
ardupilot_msgs_msg_GlobalPosition
    AP_DDS_Client::rx_global_position_control_topic{};
...
rcl_interfaces_srv_SetParameters_Response
    AP_DDS_Client::set_parameter_response{};
rcl_interfaces_srv_GetParameters_Request
    AP_DDS_Client::get_parameters_request{};
rcl_interfaces_srv_GetParameters_Response
    AP_DDS_Client::get_parameters_response{};
...
const AP_Param::GroupInfo AP_DDS_Client::var_info[] {
  ...
  // @RebootRequired: True
  // @User: Advanced
  AP_GROUPINFO_FLAGS("_ENABLE", 1, AP_DDS_Client, enabled, ENABLED_BY_DEFAULT,
      ...
      // @Description: The time in milliseconds the DDS client will wait for a
      // response from the XRCE agent before reattempting.
      // @Units: ms
      // @Range: 1 10000
      // @RebootRequired: True
      // @Increment: 1
      ...
      // @Description: The maximum number of times the DDS client will attempt
      // to ping the XRCE agent before exiting. Set to 0 to allow unlimited
      // retries.
      // @Range: 0 100
      // @RebootRequired: True
      // @Increment: 1
...
bool AP_DDS_Client::update_topic(sensor_msgs_msg_NavSatFix &msg,
  ...
  case AP_GPS::GPS_OK_FIX_3D_RTK_FIXED:
    ...
    break;
  default:
    //! @todo Can we not just use an enum class and not worry about this
    //! condition?
    break;
  }
  ...
  msg.position_covariance_type =
      (uint8_t)gps.position_covariance(instance, cov);
...
#if AP_DDS_BATTERY_STATE_PUB_ENABLED
void AP_DDS_Client::update_topic(sensor_msgs_msg_BatteryState &msg,
                                 const uint8_t instance) {
  if (instance >= AP_BATT_MONITOR_MAX_INSTANCES) {
    return;
  }
  static_assert(AP_BATT_MONITOR_MAX_INSTANCES <= 99,
                "AP_BATT_MONITOR_MAX_INSTANCES is greater than 99");
  ...
  float temperature;
  msg.temperature =
      (battery.get_temperature(temperature, instance)) ? temperature : NAN;

  float current;
  msg.current = (battery.current_amps(current, instance)) ? -1 * current : NAN;

  const float design_capacity =
      (float)battery.pack_capacity_mah(instance) * 0.001;
  ...
  msg.power_supply_health =
      (battery.overpower_detected(instance))
          ? 4
  ...
  if (battery.has_cell_voltages(instance)) {
    ...
    const uint8_t ncells_max =
        MIN(ARRAY_SIZE(msg.cell_voltage), ARRAY_SIZE(cells.cells));
...
void AP_DDS_Client::update_topic(geometry_msgs_msg_PoseStamped &msg) {
  ...
  // https://www.ros.org/reps/rep-0103.html#axis-orientation
  // As a consequence, to follow ROS REP 103, it is necessary to switch X and Y,
  // as well as invert Z (NED to ENU conversion) as well as a 90 degree rotation
  // in the Z axis for x to point forward
  Quaternion orientation;
  if (ahrs.get_quaternion(orientation)) {
    Quaternion aux(orientation[0], orientation[2], orientation[1],
                   -orientation[3]); // NED to ENU transformation
    Quaternion transformation(sqrtF(2) * 0.5, 0, 0,
                              sqrtF(2) * 0.5); // Z axis 90 degree rotation
...
void AP_DDS_Client::update_topic(geographic_msgs_msg_GeoPoseStamped &msg) {
  ...
  if (ahrs.get_location(loc)) {
    ...
    // TODO this is assumed to be absolute frame in WGS-84 as per the GeoPose
    // message definition in ROS. Use loc.get_alt_frame() to convert if
    // necessary.
...
void AP_DDS_Client::on_topic_trampoline(
    uxrSession *uxr_session, uxrObjectId object_id, uint16_t request_id,
...
void AP_DDS_Client::on_topic(uxrSession *uxr_session, uxrObjectId object_id,
  ...
  */
  ...
  case topics[to_underlying(TopicIndex::JOY_SUB)].dr_id.id: {
    const bool success =
        sensor_msgs_msg_Joy_deserialize_topic(ub, &rx_joy_topic);
  ...
  case topics[to_underlying(TopicIndex::DYNAMIC_TRANSFORMS_SUB)].dr_id.id: {
    const bool success = tf2_msgs_msg_TFMessage_deserialize_topic(
        ub, &rx_dynamic_transforms_topic);
    if (success == false) {
      break;
    }
    ...
    if (rx_dynamic_transforms_topic.transforms_size > 0) {
      ...
      GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                    "%s Received tf2_msgs/TFMessage: TF is empty", msg_prefix);
    }
    break;
  }
  ...
  case topics[to_underlying(TopicIndex::VELOCITY_CONTROL_SUB)].dr_id.id: {
    const bool success = geometry_msgs_msg_TwistStamped_deserialize_topic(
        ub, &rx_velocity_control_topic);
    if (success == false) {
      break;
    }
#if AP_EXTERNAL_CONTROL_ENABLED
    if (!AP_DDS_External_Control::handle_velocity_control(
            rx_velocity_control_topic)) {
  ...
  case topics[to_underlying(TopicIndex::GLOBAL_POSITION_SUB)].dr_id.id: {
    const bool success = ardupilot_msgs_msg_GlobalPosition_deserialize_topic(
        ub, &rx_global_position_control_topic);
    if (success == false) {
      break;
    }
    ...
    if (!AP_DDS_External_Control::handle_global_position_control(
      ...
      // TODO #23430 handle global position control failure through rosout,
      // throttled.
    }
#endif // AP_EXTERNAL_CONTROL_ENABLED
    break;
...
void AP_DDS_Client::on_request_trampoline(
    uxrSession *uxr_session, uxrObjectId object_id, uint16_t request_id,
    SampleIdentity *sample_id, ucdrBuffer *ub, uint16_t length, void *args) {
  AP_DDS_Client *dds = (AP_DDS_Client *)args;
  dds->on_request(uxr_session, object_id, request_id, sample_id, ub, length);
}

void AP_DDS_Client::on_request(uxrSession *uxr_session, uxrObjectId object_id,
                               uint16_t request_id, SampleIdentity *sample_id,
  ...
  case services[to_underlying(ServiceIndex::ARMING_MOTORS)].rep_id: {
    ...
    const bool deserialize_success =
        ardupilot_msgs_srv_ArmMotors_Request_deserialize_topic(
            ub, &arm_motors_request);
    if (deserialize_success == false) {
      break;
    }

    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s Request for %sing received",
                  msg_prefix, arm_motors_request.arm ? "arm" : "disarm");
    arm_motors_response.result =
        arm_motors_request.arm ? AP::arming().arm(AP_Arming::Method::DDS)
    ...
    const bool serialize_success =
        ardupilot_msgs_srv_ArmMotors_Response_serialize_topic(
            &reply_ub, &arm_motors_response);
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s Request for Arming/Disarming : %s",
                  msg_prefix, arm_motors_response.result ? "SUCCESS" : "FAIL");
    break;
  }
#endif // AP_DDS_ARM_SERVER_ENABLED
#if AP_DDS_MODE_SWITCH_SERVER_ENABLED
  case services[to_underlying(ServiceIndex::MODE_SWITCH)].rep_id: {
    ...
    const bool deserialize_success =
        ardupilot_msgs_srv_ModeSwitch_Request_deserialize_topic(
            ub, &mode_switch_request);
    if (deserialize_success == false) {
      break;
    }
    mode_switch_response.status = AP::vehicle()->set_mode(
        mode_switch_request.mode, ModeReason::DDS_COMMAND);
    ...
    ucdr_init_buffer(&reply_ub, reply_buffer, sizeof(reply_buffer));
    const bool serialize_success =
        ardupilot_msgs_srv_ModeSwitch_Response_serialize_topic(
            &reply_ub, &mode_switch_response);
    if (serialize_success == false) {
      break;
    }
    ...
    uxr_buffer_reply(uxr_session, reliable_out, replier_id, sample_id,
                     reply_buffer, ucdr_buffer_length(&reply_ub));
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s Request for Mode Switch : %s",
                  msg_prefix, mode_switch_response.status ? "SUCCESS" : "FAIL");
    break;
  }
#endif // AP_DDS_MODE_SWITCH_SERVER_ENABLED
#if AP_DDS_VTOL_TAKEOFF_SERVER_ENABLED
  case services[to_underlying(ServiceIndex::TAKEOFF)].rep_id: {
    ...
    const bool deserialize_success =
        ardupilot_msgs_srv_Takeoff_Request_deserialize_topic(ub,
                                                             &takeoff_request);
    if (deserialize_success == false) {
      break;
    }
    ...
    ucdr_init_buffer(&reply_ub, reply_buffer, sizeof(reply_buffer));
    const bool serialize_success =
        ardupilot_msgs_srv_Takeoff_Response_serialize_topic(&reply_ub,
                                                            &takeoff_response);
    if (serialize_success == false) {
      break;
    }
    ...
    uxr_buffer_reply(uxr_session, reliable_out, replier_id, sample_id,
                     reply_buffer, ucdr_buffer_length(&reply_ub));
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s Request for Takeoff : %s", msg_prefix,
  ...
  case services[to_underlying(ServiceIndex::PREARM_CHECK)].rep_id: {
    ...
    const bool deserialize_success =
        std_srvs_srv_Trigger_Request_deserialize_topic(ub,
                                                       &prearm_check_request);
    if (deserialize_success == false) {
      break;
    }
    prearm_check_response.success = AP::arming().pre_arm_checks(false);
    STRCPY(prearm_check_response.message, prearm_check_response.success
                                              ? "Vehicle is Armable"
    ...
    ucdr_init_buffer(&reply_ub, reply_buffer, sizeof(reply_buffer));
    const bool serialize_success =
        std_srvs_srv_Trigger_Response_serialize_topic(&reply_ub,
                                                      &prearm_check_response);
    if (serialize_success == false) {
      break;
    }
  ...
  case services[to_underlying(ServiceIndex::SET_PARAMETERS)].rep_id: {
    const bool deserialize_success =
        rcl_interfaces_srv_SetParameters_Request_deserialize_topic(
            ub, &set_parameter_request);
    if (deserialize_success == false) {
      GCS_SEND_TEXT(
          MAV_SEVERITY_ERROR,
          "%s Set Parameters Request : Failed to deserialize request.",
          msg_prefix);
      break;
    }
    ...
    for (size_t i = 0; i < set_parameter_request.parameters_size; i++) {
      ...
      // bool, string, byte_array, bool_array, integer_array, double_array and
      // string_array
      ...
      if ((parameter_flags & AP_PARAM_FLAG_INTERNAL_USE_ONLY) ||
          vp->is_read_only()) {
        set_parameter_response.results[i].successful = false;
        strncpy(set_parameter_response.results[i].reason,
      ...
      bool force_save =
          vp->set_and_save_by_name_ifchanged(param_key, param_value);
    ...
    uint32_t reply_size =
        rcl_interfaces_srv_SetParameters_Response_size_of_topic(
            &set_parameter_response, 0U);
    uint8_t reply_buffer[reply_size]{};
    ucdrBuffer reply_ub;

    ucdr_init_buffer(&reply_ub, reply_buffer, reply_size);
    const bool serialize_success =
        rcl_interfaces_srv_SetParameters_Response_serialize_topic(
    ...
    GCS_SEND_TEXT(successful_params ? MAV_SEVERITY_INFO : MAV_SEVERITY_WARNING,
                  "%s Set Parameters Request : %s", msg_prefix,
                  successful_params ? "SUCCESSFUL"
                                    : "ONE OR MORE PARAMS FAILED");
    break;
  }
  case services[to_underlying(ServiceIndex::GET_PARAMETERS)].rep_id: {
    const bool deserialize_success =
        rcl_interfaces_srv_GetParameters_Request_deserialize_topic(
            ub, &get_parameters_request);
    if (deserialize_success == false) {
      GCS_SEND_TEXT(
          MAV_SEVERITY_ERROR,
          "%s Get Parameters Request : Failed to deserialize request.",
          msg_prefix);
      break;
    }
    ...
    for (size_t i = 0; i < get_parameters_request.names_size; i++) {
      ...
      AP_Param *vp;
      char param_key[AP_MAX_NAME_SIZE + 1];
      strncpy(param_key, (char *)get_parameters_request.names[i],
              AP_MAX_NAME_SIZE);
      param_key[AP_MAX_NAME_SIZE] = 0;

      vp = AP_Param::find(param_key, &var_type);
      if (vp == nullptr) {
        get_parameters_response.values[i].type =
      ...
      case AP_PARAM_INT8: {
        ...
        get_parameters_response.values[i].integer_value =
            ((AP_Int8 *)vp)->get();
      ...
      case AP_PARAM_FLOAT: {
        ...
        get_parameters_response.values[i].double_value =
    ...
    uint32_t reply_size =
        rcl_interfaces_srv_GetParameters_Response_size_of_topic(
            &get_parameters_response, 0U);
    uint8_t reply_buffer[reply_size]{};
    ucdrBuffer reply_ub;

    ucdr_init_buffer(&reply_ub, reply_buffer, reply_size);
    const bool serialize_success =
        rcl_interfaces_srv_GetParameters_Response_serialize_topic(
    ...
    GCS_SEND_TEXT(successful_read ? MAV_SEVERITY_INFO : MAV_SEVERITY_WARNING,
                  "%s Get Parameters Request : %s", msg_prefix,
                  successful_read ? "SUCCESSFUL"
                                  : "ONE OR MORE PARAM NOT FOUND");
    break;
  }
...
void AP_DDS_Client::main_loop(void) {
  ...
  while (true) {
    if (comm == nullptr) {
      GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s transport invalid, exiting",
                    msg_prefix);
      return;
    }
    ...
    if (ping_max_retry == 0) {
      if (!uxr_ping_agent(comm, ping_timeout_ms)) {
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "%s No ping response, retrying",
                      msg_prefix);
        continue;
      }
    } else {
      if (!uxr_ping_agent_attempts(comm, ping_timeout_ms, ping_max_retry)) {
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "%s No ping response, exiting",
                      msg_prefix);
        continue;
      }
    }
    ...
    if (!init_session() || !create()) {
      GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s Creation Requests failed",
                    msg_prefix);
      return;
    }
    connected = true;
    ...
    while (connected) {
      ...
      if (cur_time_ms - last_ping_ms > DELAY_PING_MS) {
        ...
        uxr_ping_agent_session(&session, ping_agent_timeout_ms,
                               ping_agent_attempts);
      ...
      if (num_pings_missed > 2) {
        GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s No ping response, disconnecting",
                      msg_prefix);
        connected = false;
      }
    }
...
bool AP_DDS_Client::init_transport() {
  ...
  if (!initTransportStatus) {
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s Transport initialization failed",
                  msg_prefix);
    return false;
  }
...
bool AP_DDS_Client::init_session() {
  ...
  uxr_set_request_callback(&session, AP_DDS_Client::on_request_trampoline,
                           this);

  while (!uxr_create_session(&session)) {
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s Initialization waiting...",
                  msg_prefix);
...
bool AP_DDS_Client::create() {
  ...
  const uint16_t requestsParticipant[nRequestsParticipant] = {
      participant_req_id};
  ...
  if (!uxr_run_session_until_all_status(&session, requestTimeoutParticipantMs,
                                        requestsParticipant, statusParticipant,
                                        nRequestsParticipant)) {
    GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s Participant session request failure",
                  msg_prefix);
  ...
  for (uint16_t i = 0; i < ARRAY_SIZE(topics); i++) {
    ...
    if (topics[i].topic_rw == Topic_rw::DataWriter) {
      // Publisher
      const uxrObjectId pub_id = {.id = topics[i].pub_id,
      ...
      const auto dwriter_req_id = uxr_buffer_create_datawriter_bin(
          &session, reliable_out, topics[i].dw_id, pub_id, topic_id,
          topics[i].qos, UXR_REPLACE);
      ...
      if (!uxr_run_session_until_all_status(&session, requestTimeoutMs,
                                            requests, status, nRequests)) {
        GCS_SEND_TEXT(
            MAV_SEVERITY_ERROR,
            "%s Topic/Pub/Writer session request failure for index '%u'",
            msg_prefix, i);
        for (uint8_t s = 0; s < nRequests; s++) {
          GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s Status '%d' result '%u'",
                        msg_prefix, s, status[s]);
        }
        // TODO add a failure log message sharing the status results
        return false;
      } else {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                      "%s Topic/Pub/Writer session pass for index '%u'",
                      msg_prefix, i);
      }
    } else if (topics[i].topic_rw == Topic_rw::DataReader) {
      // Subscriber
      const uxrObjectId sub_id = {.id = topics[i].sub_id,
      ...
      const auto dreader_req_id = uxr_buffer_create_datareader_bin(
          &session, reliable_out, topics[i].dr_id, sub_id, topic_id,
          topics[i].qos, UXR_REPLACE);
      ...
      if (!uxr_run_session_until_all_status(&session, requestTimeoutMs,
        ...
        GCS_SEND_TEXT(
            MAV_SEVERITY_ERROR,
            "%s Topic/Sub/Reader session request failure for index '%u'",
            msg_prefix, i);
        for (uint8_t s = 0; s < nRequests; s++) {
          GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s Status '%d' result '%u'",
                        msg_prefix, s, status[s]);
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                      "%s Topic/Sub/Reader session pass for index '%u'",
                      msg_prefix, i);
        uxr_buffer_request_data(&session, reliable_out, topics[i].dr_id,
                                reliable_in, &delivery_control);
      }
    }
  }
  ...
  for (uint16_t i = 0; i < ARRAY_SIZE(services); i++) {
    ...
    if (services[i].service_rr == Service_rr::Replier) {
      ...
      if (!uxr_run_session_until_all_status(&session, requestTimeoutMs,
        ...
        GCS_SEND_TEXT(
            MAV_SEVERITY_ERROR,
            "%s Service/Replier session request failure for index '%u'",
            msg_prefix, i);
        GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "%s Status result '%u'", msg_prefix,
                      status);
        // TODO add a failure log message sharing the status results
        return false;
      } else {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                      "%s Service/Replier session pass for index '%u'",
                      msg_prefix, i);
        uxr_buffer_request_data(&session, reliable_out, rep_id, reliable_in,
                                &delivery_control);
      }
...
void AP_DDS_Client::write_time_topic() {
  ...
  if (connected) {
    ...
    const uint32_t topic_size =
        builtin_interfaces_msg_Time_size_of_topic(&time_topic, 0);
    uxr_prepare_output_stream(&session, reliable_out,
                              topics[to_underlying(TopicIndex::TIME_PUB)].dw_id,
                              &ub, topic_size);
    const bool success =
        builtin_interfaces_msg_Time_serialize_topic(&ub, &time_topic);
    if (!success) {
...
void AP_DDS_Client::write_nav_sat_fix_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
        sensor_msgs_msg_NavSatFix_size_of_topic(&nav_sat_fix_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::NAV_SAT_FIX_PUB)].dw_id, &ub,
        topic_size);
    const bool success =
        sensor_msgs_msg_NavSatFix_serialize_topic(&ub, &nav_sat_fix_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
...
void AP_DDS_Client::write_static_transforms() {
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::STATIC_TRANSFORMS_PUB)].dw_id, &ub,
        topic_size);
    const bool success = tf2_msgs_msg_TFMessage_serialize_topic(
        &ub, &tx_static_transforms_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
    }
...
void AP_DDS_Client::write_battery_state_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
        sensor_msgs_msg_BatteryState_size_of_topic(&battery_state_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::BATTERY_STATE_PUB)].dw_id, &ub,
        topic_size);
    const bool success =
        sensor_msgs_msg_BatteryState_serialize_topic(&ub, &battery_state_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
    }
...
void AP_DDS_Client::write_local_pose_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
        geometry_msgs_msg_PoseStamped_size_of_topic(&local_pose_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::LOCAL_POSE_PUB)].dw_id, &ub,
        topic_size);
    const bool success =
        geometry_msgs_msg_PoseStamped_serialize_topic(&ub, &local_pose_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
    }
...
void AP_DDS_Client::write_tx_local_velocity_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size = geometry_msgs_msg_TwistStamped_size_of_topic(
        &tx_local_velocity_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::LOCAL_VELOCITY_PUB)].dw_id, &ub,
        topic_size);
    const bool success = geometry_msgs_msg_TwistStamped_serialize_topic(
        &ub, &tx_local_velocity_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
    }
...
void AP_DDS_Client::write_tx_local_airspeed_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size = geometry_msgs_msg_Vector3Stamped_size_of_topic(
        &tx_local_airspeed_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::LOCAL_AIRSPEED_PUB)].dw_id, &ub,
        topic_size);
    const bool success = geometry_msgs_msg_Vector3Stamped_serialize_topic(
        &ub, &tx_local_airspeed_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
    }
...
void AP_DDS_Client::write_imu_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
        sensor_msgs_msg_Imu_size_of_topic(&imu_topic, 0);
    uxr_prepare_output_stream(&session, reliable_out,
                              topics[to_underlying(TopicIndex::IMU_PUB)].dw_id,
                              &ub, topic_size);
...
void AP_DDS_Client::write_geo_pose_topic() {
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::GEOPOSE_PUB)].dw_id, &ub, topic_size);
    const bool success = geographic_msgs_msg_GeoPoseStamped_serialize_topic(
        &ub, &geo_pose_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
...
void AP_DDS_Client::write_clock_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
        rosgraph_msgs_msg_Clock_size_of_topic(&clock_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::CLOCK_PUB)].dw_id, &ub, topic_size);
    const bool success =
        rosgraph_msgs_msg_Clock_serialize_topic(&ub, &clock_topic);
    if (!success) {
      // TODO sometimes serialization fails on bootup. Determine why.
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
    }
...
void AP_DDS_Client::write_gps_global_origin_topic() {
  ...
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
        geographic_msgs_msg_GeoPointStamped_size_of_topic(
            &gps_global_origin_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::GPS_GLOBAL_ORIGIN_PUB)].dw_id, &ub,
        topic_size);
    const bool success = geographic_msgs_msg_GeoPointStamped_serialize_topic(
...
void AP_DDS_Client::update() {
  ...
  if (cur_time_ms - last_battery_state_time_ms > DELAY_BATTERY_STATE_TOPIC_MS) {
    for (uint8_t battery_instance = 0;
         battery_instance < AP_BATT_MONITOR_MAX_INSTANCES; battery_instance++) {
...
#endif // AP_DDS_LOCAL_POSE_PUB_ENABLED
#if AP_DDS_LOCAL_VEL_PUB_ENABLED
  if (cur_time_ms - last_local_velocity_time_ms >
      DELAY_LOCAL_VELOCITY_TOPIC_MS) {
...
#endif // AP_DDS_CLOCK_PUB_ENABLED
#if AP_DDS_GPS_GLOBAL_ORIGIN_PUB_ENABLED
  if (cur_time_ms - last_gps_global_origin_time_ms >
      DELAY_GPS_GLOBAL_ORIGIN_TOPIC_MS) {
